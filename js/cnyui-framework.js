/**
 *CNY信息技术有限公司 Copyright © CNY 2014
 */
//样式
$(function () {
    readyIndex();
...
自动给页面上所有表单控件设置value
*/
//data是键值对json，键对应表单控件id，值对应表单控件值
...
function IsNumber(objId) {
    $("#" + objId).bind("contextmenu", function () {
        return false;
    });
    $("#" + objId).css('ime-mode', 'disabled');
...
*/
//_html弹出框正文的html
//_id弹出框实例id，_title标题，_width，_height，，
//_callBack在弹出框按确定按钮后的回调函数，接收data参数为
...
/*带数据下拉菜单控件(数据是表格形式) begin====================================*/
/*为指定的dom(如：input#Objkey)提供一个单击下拉的数据菜单。
其中Objkey为dom的id，width、height为下拉菜单的宽高(注意要带单位px)，
data为下来数据的html字符串并且是"<tr><td>数据...</td></tr><tr><td>数据...</td></tr>"
callBack为下拉菜单单项单击后进行的回调函数，即选中后。回调传递的参数data为选中行的td的id和text组成key:val的Json
在 订单>新增 可用到
 */
...
function autocomplete(Objkey, width, height, data, callBack) {
    ...
    //$("#div_gridshow").css("left", Y).css("top", X + $('#' + Objkey).height()).show();//cny_note
    $("#div_gridshow").css("left", Y).css("top", X + $('#' + Objkey).height()).hide();//cny_add
...
function comboBoxTree(Objkey, height) {
    ...
    $("#" + Objkey).css('ime-mode', 'disabled');//禁用IME
    $("#" + Objkey).bind("contextmenu", function () { return false; }); //绑定右键菜单
...
/**
 ...
 Objkey:          ID
 height：         高度
 WDTreeSettings： WDTree初始化设置的对象
 **/
function comboBoxDataTree(Objkey, height,WDTreeSettings) {
    top.$(".ui_state_highlight").focus();
    var width = $("#" + Objkey).width();
    $("#" + Objkey).css('ime-mode', 'disabled');//禁用IME
    $("#" + Objkey).bind("contextmenu", function () { return false; }); //绑定右键菜单
    ...
    var comboBoxTree = "comboBoxTree" + Objkey;
    if ($("#" + comboBoxTree).attr("id") == undefined) {
        $('body').append('<div id="' + comboBoxTree + '" style="overflow: auto;border: 1px solid #ccc;border-top:none;width:' + width + 'px;height:' + height + 'px;position: absolute; background-color: #fff; display: none;"></div>');
    }
    $("#" + comboBoxTree).css("left", Y).css("top", X + $("#" + Objkey).height() + 2).css("z-index", "99").hide();

    $("#comboBoxTree" + Objkey).treeview(WDTreeSettings);

    //任意键关闭
    //document.onclick = function (e) {
    $(document).click(function (e) {//用事件监听而不是注册，避免其它方法添加的事件监听被覆盖
        var e = e ? e : window.event;
        var tar = e.srcElement || e.target;
        if (tar.id != '' + comboBoxTree + '') {
            if ($(tar).hasClass("bbit-tree-ec-icon")) {
...
function delConfirm(url, parm, msg) {
    confirmDialog("CNY提示", msg, function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
...
function delConfig(url, parm, count) {
    ...
    confirmDialog("CNY提示", "注：您确定要删除 " + count + " 笔记录？", function (r) {
        if (r) {
            Loading(true, "正在删除数据...");
            window.setTimeout(function () {
                AjaxJson(url, parm, function (data) {
...
function GetDeriveExcel(obj, fileName) {
    ...
    confirmDialog("CNY提示", "注：您确定要导出 " + data.length + " 笔记录？", function (r) {
        if (r) {
            var postData = {
...
/*绑定下拉框
ControlId:绑定控件的JQ选择器字符串
Memo:默认显示
DataJson: 数据数组Json，注意是字符串格式的
*/
...
//=================动态菜单tab标签========================
/*
* tabid:用于设置新的tab页签的id="tabs_"+tabid  新的tab页面iframe的id="tabs_iframe_"+tabid
* url:链接页面地址，绝对地址。完整地址
* name：tab页签上的名称
* img：tab页签上的图标
* Isclose：是否带关闭按钮
* IsReplace：如果已有tabid对应的tab是替换，还是另外开一个tab
* IsVisitorModule：该tab对应的模块是否被访问过，如果没有则写入后台日志
* */
function AddTabMenu(tabid, url, name, img, Isclose, IsReplace, IsVisitorModule) {
    ...
    if (top.document.getElementById("tabs_" + tabid) == null) { //如果当前tabid存在直接显示已经打开的tab
        ...
        if (Isclose != 'false') { //判断是否带关闭tab
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class='selected' win_close='true'><span title='" + name + "' onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','true')\"><a><img src='" + img + "' width='16' height='16'>" + name + "</a></span><a class=\"win_close\" title=\"关闭当前窗口\" onclick=\"RemoveDiv('" + tabid + "')\"></a></li>");
        } else {
            tabs_container.append("<li id=\"tabs_" + tabid + "\" class=\"selected\" onclick=\"AddTabMenu('" + tabid + "','" + url + "','" + name + "','false')\"><a><img src='" + img + "' width='16' height='16'>" + name + "</a></li>");
        }
...
//关闭事件
//obj为tab对应模块的id，tab页签和页面都有此模块id+其它字符串组成的
...
function VisitorModule(ModuleId, ModuleName) {
    alert('记录用户操作日志(访问时)：模块ID：'+ModuleId+'模块名：'+ModuleName);
    //根据实际部署修改以下getAjax的url地址。
    //top.$("#ModuleId").val(ModuleId);//在界面上的tab页签栏放置一个隐藏域存储当前打开的页签页面
    //getAjax("/Home/SetModuleId", { ModuleId: ModuleId, ModuleName: ModuleName }, function (data) { })//记录模块访问操作到数据库日志表
}
//离开模块
function SetLeave(ModuleId, ModuleName) {
    //记录用户操作日志，离开模块
    alert('记录用户操作日志(离开时)：模块ID：'+ModuleId+'模块名：'+ModuleName);
    //根据实际部署修改以下getAjax的url地址。
