...
function readyIndex() {
    ...
    $("#topnav .droppopup a").hover(function () {
        ...
        $(this).addClass('onnav');
        var Y = $(this).attr("offsetLeft");//和浏览器左边的距离
        $(this).find('.popup').show().css('top', ($(this).offset().top + 71)).css('left', $(this).offset().left - ($(this).find('.popup').width() / 2 - 36));//设置下拉列表div位置及显示
    }, function () {
        $("#topnav .droppopup a").removeClass('onnav');
        $(this).find('.popup').hide();//隐藏下拉列表div
    });
...
/**安全退出**/
function IndexOut() {
    var msg = "<div class='ui_alert'>确认要退出CNY前端框架？</div>"
    top.$.dialog({
        id: "confirmDialog",
        lock: true,
        icon: "hits.png",
...
function PersonCenter() {
    var url = "/FrontEndFramework/Views/testDialog.html";
    Dialog(url, "ResetPassword", "个人中心", 750, 400);
}
//快捷方式列表
function ShortcutsList() {
    $("#Shortcuts").html('');
    //AjaxJson("/", {}, function (DataJson) {
    var DataJson = [{ModuleId:'test',Location:'http:www.baidu.com',FullName:'testname',Icon:'http:www.baidu.com/test.png'}];
...
function Shortcuts() {
    ...
    openDialog(url, "Shortcut", "快捷方式设置", 500, 300, function (iframe) {
        top.frames[iframe].AcceptClick()// 调用子页面的AcceptClick方法
    });
}
//页面关闭事件
function PageClose() {
...
//技术支持
function Support() {
    Dialog("/FrontEndFramework/Views/testDialog.html", "Support", "7 × 24 技术支持服务", 600, 275);
}
//关于我们
function About() {
    alertDialog("CNY前端框架<br>版本0.0.1<br>CNY信息技术有限公司<br>保留所有权利", 0);
}
//个性化皮肤设置
function SkinIndex() {
    Dialog("/FrontEndFramework/Views/testDialog.html", "SkinIndex", "个性化设置", 580, 350);
}